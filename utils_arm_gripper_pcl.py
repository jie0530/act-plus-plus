...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path_list, camera_names, pcl_type, norm_stats, episode_ids, episode_len, chunk_size, policy_class):
    ...
    def __getitem__(self, index):
        ...
        try:
            with h5py.File(dataset_path, 'r') as root:
                ...
                if f'/observations/pointcloud_{self.pcl_type}' in root:
                    pointcloud_data = {}
                    pc_path = f'/observations/pointcloud_{self.pcl_type}/cam_high'
                    if pc_path in root:
                        # 获取原始点云数据
                        ...
                        if len(xyz) > 23000:
                            try:
                                ...
                                xyz = xyz[sampling_idx]
                                rgb = rgb[sampling_idx]
                            except ImportError:
                                # 如果没有fpsample模块，使用随机采样
                                indices = np.random.choice(len(xyz), 23000, replace=False)
                                xyz = xyz[indices]
                                rgb = rgb[indices]
                        elif len(xyz) < 23000:
                            # 如果点数不足，进行填充
                            padding_size = 23000 - len(xyz)
                            xyz_padding = np.zeros((padding_size, 3))
                            rgb_padding = np.zeros((padding_size, 3))
                            xyz = np.vstack([xyz, xyz_padding])
                            rgb = np.vstack([rgb, rgb_padding])
        ...
        return qpos_data, action_data, is_pad, pointcloud_data
...
def load_data(dataset_dir_l, name_filter, camera_names, pcl_type, batch_size_train, batch_size_val, chunk_size, skip_mirrored_data=False, load_pretrain=False, policy_class=None, stats_dir_l=None, sample_weights=None, train_ratio=0.99):
    ...
    train_dataset = EpisodicDataset(dataset_path_list, camera_names, pcl_type, norm_stats, train_episode_ids, train_episode_len, chunk_size, policy_class)
    val_dataset = EpisodicDataset(dataset_path_list, camera_names, pcl_type, norm_stats, val_episode_ids, val_episode_len, chunk_size, policy_class)
