...
DATA_DIR = '/home/wsco/jie_ws/src/act-plus-plus/data'
print("\n\n\n\n", DATA_DIR, "\n\n\n")
...
# __file__: 表示当前 Python 脚本文件的路径。
# pathlib.Path(__file__): 使用 pathlib 模块将 __file__ 转换为 Path 对象，便于进行路径操作。
# .parent: 获取当前文件的父目录。
# .resolve(): 将路径转换为绝对路径，确保路径正确解析（包括符号链接）。
# str(...): 将路径转换为字符串形式，以便打印或进一步处理。
