...
def main():
    ...
    t1 = rospy.Time.now()
    print("开始采集")
    recorder,max_t = ros_record_data.get_recorder("aloha_pick_trans_bottle")
    ...
    while continue_collection and not rospy.is_shutdown():
        ...
        while len(recorder.record_data_dict['/observations/qpos']) < recorder.max_timesteps:
            print("len(recorder.record_data_dict['/observations/qpos'])",len(recorder.record_data_dict['/observations/qpos']))
            rospy.sleep(0.1)
            # print("sleep")
            # # 增加停止单次收集的键盘控制
